...
public class ChunkMesh extends BaseMesh {
    ...
    public ChunkMesh(Chunk chunk, Level level) {
        this.level = level;
        this.chunk = chunk;
        this.quality = new VerticesBuffer[Quality.values().length - 1];
        this.currentQuality = Quality.NOT_RENDERED;
    ...
    @Override
    public void bake() {
        if (this.currentQuality != Quality.NOT_RENDERED) {
    ...
    public void setVerticesQuality(Quality quality, VerticesBuffer buffer) {
